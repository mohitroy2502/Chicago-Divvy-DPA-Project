...
Dst$start_time <- parse_date_time(Dst$start_time, orders = c("%m/%d/%Y %H:%M:%S","%m/%d/%Y %H:%M", "%Y-%m-%d %H:%M", "%Y-%m-%d %H:%M:%S"))
Dst$end_time <- parse_date_time(Dst$end_time, orders = c("%m/%d/%Y %H:%M:%S","%m/%d/%Y %H:%M", "%Y-%m-%d %H:%M", "%Y-%m-%d %H:%M:%S"))

# Calculating trip duration based on start and end time
Dst$tripduration = difftime(Dst$end_time, Dst$start_time,units = "secs")
