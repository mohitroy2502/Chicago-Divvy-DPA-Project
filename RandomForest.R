...
v <- data.frame(Train$TotalDuration, Train$No.Of.Trips, Train$Weather_Icon)
v <- setNames(v, c("Test.TotalDuration", "Test.No.Of.Trips", "Test.Weather_Icon"))
...
y_pred2 = predict(classifier2, newdata = data.frame(Test$TotalDuration, Test$No.Of.Trips, Test$Weather_Icon))

cm2 = confusionMatrix(reference = Test$Maintenance, data = y_pred2)
cm2
...
# # Logistic Regression
# library(stats)
# model2= glm(formula = Maintenance ~ bikeid + TotalDuration + Weather_Icon + TotalTrips, family=binomial, data = Train)
# 
